...
const divisible_by_three = pokémon.filter(p => p.id % 3 === 0);
...
const names_greater = pokémon.filter(p => p.id > 99).map(p => p.name)
console.log(names_greater)
...
const fly = pokémon.filter(p => p.types.includes("flying")).map(p => p.types[0])
console.log(fly)
